...
### 1) Define scope and question (start small)
...
### 2) Plan a multi‑source search (what is a search, really?)
...
### From PICO to a searchable question (beginner‑friendly)
...
### From protocol to screening and extraction (one page)
...
## Database‑aware tactics (without jargon)
...
### C. Active questions while reading
...
::: {.columns}
::: {.column width="55%"}
Appraise quality: venue, data provenance, method soundness, transparency, generalisability. Then synthesise: group by method/data/domain; state agreements, disagreements, likely reasons, and gaps. Finally, write with a simple structure: Introduction (scope & question), Themed body (comparative analysis), Limitations, Conclusion (known/contested/unknown; why your project now).
:::
...
## What “credible” looks like (beginner checklist)
...
## Scopus-based mini‑assignment (60 minutes)
...
What you’ll produce
- A one‑page note (or 3–4 slides) with: top topics, 3 highly‑cited papers per area with a one‑sentence finding each, and 1–2 suggested contacts per area (authors at your university).

Set‑up (2 minutes)
- Log into Scopus via your library. If Scopus is unavailable, use Google Scholar as a fallback (see “No Scopus?” below).

Step 1 — Filter to your university (5 minutes)
- In Scopus Advanced search, add an affiliation filter for your institution.
- Replace <Your University> below with your institution’s name exactly as Scopus records it (use the Affiliation search if unsure).

```text
AFFIL("Harper Adams University")
```

Step 2 — Three quick topic scans (3 × 10 minutes = 30 minutes)
...
- 1–2 local authors (from your university) who appear repeatedly in this area
...
Step 3 — Pick contacts (8 minutes)
- From your shortlists, pick 1–2 authors per area who (a) have multiple papers and (b) are current staff.
- Record their names, department/school, and a portfolio link if available.

Step 4 — Package your note (5 minutes)
...
No Scopus? (fallback, 10 minutes for any one area)
...
Marking (lightweight)
- Completeness: 3 papers × 3 areas with one‑line findings; at least one plausible contact per area
- Judgement: choices make sense (recent/influential, match the area)
- Clarity: short, readable notes a supervisor could skim

Time plan (60 minutes)
- 2 min set‑up; 5 min affiliation; 30 min three scans; 8 min contacts; 5 min packaging; (buffer 10 min if needed)
